...
public class CultTower extends ApplicationAdapter {
	...
	public void create() {
		...
		this.animation = new Animation(0.5f, this.animationFrames[0]);
	...
	public void render() {
		...
		//Guards
		if (!npc4Dead) {
			this.main.draw((TextureRegion) animation.getKeyFrame(elaspedTime, true),
	...
	private void touchObjects() {
		if (mainChar.rect.overlaps(door1.rect) && locked) {
			environment.draw(main, (GlyphLayout) door1.getText(0), door1.getX() + 2, door1.getY() - 2);
		} else if (mainChar.rect.overlaps(door1.rect) && !locked) {
			environment.draw(main, (GlyphLayout) door1.getText(1), door1.getX() + 2, door1.getY() - 2);
		}
